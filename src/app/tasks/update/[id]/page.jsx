...
export default function UpdateTaskPage() {
  ...
  const { register, handleSubmit, reset } = useForm({
    defaultValues: { title: "", description: "", priority: "Low", status: "To Do", assignedUser: "", dueDate: "" },
  });
    ...
    const fetchData = async () => {
      try {
        ...
        const [usersRes, taskRes] = await Promise.all([
          axios.get("https://task-mate-server-iota.vercel.app/users", { headers: { Authorization: `Bearer ${token}` } }),
          axios.get(`https://task-mate-server-iota.vercel.app/tasks/${id}`, { headers: { Authorization: `Bearer ${token}` } }),
        ]);
        setUsers(usersRes.data);
        const task = taskRes.data;
        reset({ title: task.title, description: task.description, priority: task.priority, status: task.status, assignedUser: task.assignedUser, dueDate: task.dueDate?.split("T")[0] || "" });
      } catch {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await axios.patch(`https://task-mate-server-iota.vercel.app/tasks/${id}`, data, { headers: { Authorization: `Bearer ${session.access_token}` } });
  ...
  if (loading) return <div className="text-center py-10 text-gray-500">Loading task data...</div>;
